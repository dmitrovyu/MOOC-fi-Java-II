...
public class HashMap<K, V> {
    ...
    private void copy(ArrayList<Pair<K, V>>[] myList, int fromIdx) {
        for (int i = 0; i < this.values[fromIdx].size(); i++) {
            Pair<K, V> value = this.values[i].get(i);
            int hashValue = Math.abs(value.getKey().hashCode() % myList.length);
            if (myList[hashValue] == null) {
                myList[hashValue] = new ArrayList<>();

            }
            myList[hashValue].add(value);
        }
    }
